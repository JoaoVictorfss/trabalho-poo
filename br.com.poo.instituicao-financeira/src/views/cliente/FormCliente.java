...
public class FormCliente extends JPanel {
	...
	public FormCliente() {
		...
		botao.addActionListener(event -> {
			...
			if(vazio(campoNome.getText()))this.nomeCliente = campoNome.getText();
			if(vazio( campoCpf.getText()))this.cpfCliente =  campoCpf.getText();
			if(vazio( campoDataNasc.getText())) transformaData(campoDataNasc.getText());
			if(vazio(campoPais.getText()))this.endPais = campoPais.getText();
			if(vazio(campoUf.getText()))this.endUf = campoUf.getText();
			if(vazio(campoCep.getText()))this.endCep = campoCep.getText();
			if(vazio(campoCidade.getText()))this.endCidade = campoCidade.getText();
			if(vazio(campoRua.getText()))this.endRua = campoRua.getText();
			if(vazio(campoNumero.getText())); this.endNumero = transformaNumero(campoNumero.getText());
			if(vazio(campoNroConta.getText())); this.endNumero = transformaNumero(campoNroConta.getText());
			if(vazio(campoEscolaridade.getText())) this.escolaridade = campoEscolaridade.getText();
			if(vazio(campoEstadoCiv.getText())) this.estadoCiv = campoEstadoCiv.getText();
			...
			if(this.valido) {
				...
				//Mostrar alerta informando que os dados estão incorretos
				return;
			}
		});
	}
    ...
    private void transformaData(String data) {
        ...
        try {
        	...
        	//colocar alerta informando que a data está errada
        	this.valido = false;
    ...
    private int transformaNumero(String nro) {
    	try {
        	...
        	this.valido = false;
        	//mostra alerta que o valor no número é inválido
